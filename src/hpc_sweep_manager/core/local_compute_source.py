...
class LocalComputeSource(ComputeSource):
    ...
    async def submit_job(
        ...
        try:
            ...
            logger.info(
                f"Job {job_name} submitted to local source {self.name} with ID {job_id}"
            )
            return job_id

        except Exception as e:
            logger.error(
                f"Failed to submit job {job_name} to local source {self.name}: {e}"
            )
            raise
    ...
    async def collect_results(self, job_ids: Optional[List[str]] = None) -> bool:
        ...
        try:
            logger.debug(f"Local results already available in sweep directory")
            return True
    ...
    async def cleanup(self):
        ...
        try:
            if self.local_manager:
                ...
                if self.active_jobs:
                    logger.info(
                        f"Cancelling {len(self.active_jobs)} remaining local jobs"
                    )
    ...
    def __str__(self) -> str:
        return (
            f"Local:{self.name}: {self.current_job_count}/{self.max_parallel_jobs} jobs"
        )
