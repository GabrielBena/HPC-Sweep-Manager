...
async def create_ssh_connection(
    host: str, ssh_key: Optional[str] = None, ssh_port: int = 22
):
    ...
    if ssh_key:
        ...
        if expanded_key:
            ...
            logger.warning(
                f"Specified SSH key not found: {ssh_key}, using default SSH behavior"
            )
    else:
    ...
    try:
        ...
        if "client_keys" in connection_kwargs:
            ...
            simple_kwargs = {
                k: v for k, v in connection_kwargs.items() if k != "client_keys"
            }
            try:
...
class RemoteDiscovery:
    ...
    def __init__(self, local_hsm_config: Dict[str, Any]):
        ...
        if not ASYNCSSH_AVAILABLE:
            raise ImportError(
                "asyncssh is required for distributed sweeps. "
                "Install with: pip install asyncssh"
            )

    async def discover_remote_config(
        ...
        try:
            ...
            async with await create_ssh_connection(
                ...
                remote_hsm_config = await self._read_remote_hsm_config(
                    conn, hsm_config_path
                )
                if not remote_hsm_config:
                    logger.error(
                        f"Failed to read hsm_config.yaml on remote {remote_name}"
                    )
                    return None

                # 3. Extract paths and configuration
                remote_config = self._extract_remote_paths(
                    remote_config, remote_hsm_config
                )
                ...
                if not is_valid:
                    logger.error(
                        f"Remote environment validation failed for {remote_name}"
                    )
                    return None
                ...
                logger.info(
                    f"Successfully discovered configuration for remote: {remote_name}"
                )
                return remote_config

        except Exception as e:
            logger.error(
                f"Failed to discover remote configuration for {remote_name}: {e}"
            )
            return None
    ...
    async def _read_remote_hsm_config(
        self, conn, config_path: str
    ...
    async def _validate_remote_environment(
        self, conn, remote_config: RemoteConfig
    ) -> bool:
        ...
        if remote_config.train_script and remote_config.project_root:
            ...
            if remote_config.train_script.startswith("/"):
                ...
                script_path = (
                    f"{remote_config.project_root}/{remote_config.train_script}"
                )

            validation_tasks.append(
                self._check_path(conn, script_path, "Training script")
            )
        ...
        if remote_config.config_dir:
            ...
            if remote_config.config_dir.startswith("/"):
                ...
                if remote_config.project_root:
                    config_path = (
                        f"{remote_config.project_root}/{remote_config.config_dir}"
                    )
                else:
                    config_path = remote_config.config_dir

            validation_tasks.append(
                self._check_path(conn, config_path, "Config directory")
            )
        ...
        logger.info(
            f"Environment validation: {success_count}/{total_checks} checks passed"
        )
    ...
    async def discover_all_remotes(
        self, remotes_config: Dict[str, Any]
