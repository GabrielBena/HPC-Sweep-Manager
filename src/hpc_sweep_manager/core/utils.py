...
def count_jobs_in_queue() -> int:
    ...
    try:
        # Try PBS first
        result = subprocess.run(["qstat", "-u", os.getenv("USER")], capture_output=True, text=True, timeout=10)
    ...
    try:
        # Try Slurm
        result = subprocess.run(["squeue", "-u", os.getenv("USER")], capture_output=True, text=True, timeout=10)
        if result.returncode == 0:
            # Count lines (excluding header)
            lines = result.stdout.strip().split("\n")
