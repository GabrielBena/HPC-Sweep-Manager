...
from .remote_discovery import RemoteDiscovery, RemoteConfig
...
class DistributedJobManager:
    ...
    async def setup_all_sources(self, sweep_id: str) -> bool:
        ...
        logger.info(
            f"Setting up {len(self.sources)} compute sources for distributed execution"
        )
        ...
        if failed_sources:
            ...
            self.source_by_name = {
                name: s
                for name, s in self.source_by_name.items()
                if name in successful_sources
            }

        logger.info(
            f"✓ {len(successful_sources)} compute sources ready for distributed execution"
        )
        return True
    ...
    async def submit_distributed_sweep(
        ...
        self._collection_task = asyncio.create_task(
            self._collect_results_continuously()
        )
    ...
    async def _monitor_jobs(self):
        ...
        while self._running and not self._cancelled:
            try:
                ...
                if (
                    self.jobs_completed != old_completed
                    or self.jobs_failed != old_failed
                ):
                    logger.debug(
    ...
    async def _collect_results_continuously(self):
        ...
        while self._running and not self._cancelled:
            try:
                ...
                ssh_sources = [
                    s for s in self.sources if isinstance(s, SSHComputeSource)
                ]
                    ...
                    for source in ssh_sources:
                        ...
                        if completed_job_ids:
                            collection_tasks.append(
                                source.collect_results(completed_job_ids)
                            )
    ...
    def _update_progress_stats(self):
        ...
        completed = sum(
            1 for job in self.all_jobs.values() if job.status == "COMPLETED"
        )
        failed = sum(1 for job in self.all_jobs.values() if job.status == "FAILED")
        cancelled = sum(
            1 for job in self.all_jobs.values() if job.status == "CANCELLED"
        )
