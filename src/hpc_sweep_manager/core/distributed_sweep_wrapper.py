...
from .distributed_job_manager import (
    DistributedJobManager,
    DistributedSweepConfig,
    DistributionStrategy,
)
from .local_compute_source import LocalComputeSource
from .ssh_compute_source import SSHComputeSource
...
class DistributedSweepWrapper:
    ...
    def __init__(
        self, hsm_config, console, logger, sweep_dir: Path, show_progress: bool = True
    ):
    ...
    async def _async_submit_sweep(
        ...
        try:
            self.console.print(
                f"[cyan]Setting up distributed computing environment...[/cyan]"
            )
            ...
            if not setup_success:
                raise Exception(
                    "Failed to setup compute sources for distributed execution"
                )

            self.console.print(f"[green]✓ Distributed environment ready[/green]")
            ...
            self.console.print(
                f"[green]✓ Distributed sweep completed successfully![/green]"
            )
            return job_ids
    ...
    def _create_distributed_config(self) -> DistributedSweepConfig:
        ...
        config = DistributedSweepConfig(
            ...
            enable_interactive_sync=distributed_config.get(
                "enable_interactive_sync", True
            ),
        )
    ...
    async def _setup_compute_sources(self):
        ...
        self.logger.info(
            f"Configured {len(self.distributed_manager.sources)} compute sources"
        )
    ...
    async def _add_local_compute_source(self, distributed_config: Dict[str, Any]):
        ...
        try:
            ...
            python_path = (
                self.hsm_config.get_default_python_path()
                or detector.detect_python_path()
            )
            script_path = (
                self.hsm_config.get_default_script_path()
            ...
            if self.hsm_config:
                local_max_jobs = min(
                    local_max_jobs, self.hsm_config.get_max_array_size() or 4
                )
            ...
            self.console.print(
                f"[yellow]⚠ Could not add local compute source: {e}[/yellow]"
            )
    ...
    async def _add_ssh_compute_sources(self, remotes: Dict[str, Any]):
        ...
        for remote_name, remote_config in remotes.items():
            if not remote_config.get("enabled", True):
                self.console.print(
                    f"[dim]- Skipping disabled source: {remote_name}[/dim]"
                )
                continue

            try:
                self.console.print(
                    f"[cyan]Discovering SSH source: {remote_name}...[/cyan]"
                )
                ...
                if discovered_config:
                    ...
                    self.logger.warning(
                        f"Failed to discover configuration for {remote_name}"
                    )
                    self.console.print(
    ...
    async def _async_collect_results(self) -> bool:
        ...
        try:
            ...
            ssh_sources = [
                s
                for s in self.distributed_manager.sources
                if isinstance(s, SSHComputeSource)
            ]
            ...
            if ssh_sources:
                ...
                self.console.print(
                    f"[green]✓ Results are local (no remote sources)[/green]"
                )
                return True
...
def create_distributed_sweep_wrapper(
    ...
    if not distributed_config.get("remotes") and not distributed_config.get(
        "local_max_jobs"
    ):
        raise Exception("No compute sources configured for distributed execution")
