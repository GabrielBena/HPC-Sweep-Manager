...
def validate_cmd(ctx, config: Optional[Path], sweep_config: Optional[Path]):
    ...
    try:
        ...
        if not config and not sweep_config:
            ...
            sweep_config_path = Path("sweeps/sweep.yaml")
...
def _create_sweep_infrastructure(
    ...
    try:
        ...
        example_sweep_path = sweeps_dir / "sweep.yaml"
...
2. **Create sweep config**: Edit `sweeps/sweep.yaml` for your parameters
3. **Test the setup**: Run `hsm run --config sweeps/sweep.yaml --dry-run`
4. **Submit jobs**: Run `hsm run --config sweeps/sweep.yaml --sources local`

[bold]Useful Commands:[/bold]
- `hsm config validate` - Validate configuration files
- `hsm config show` - Display current configuration  
- `hsm run --help` - Show sweep execution options
